...
flowchart TB
    ...
    M["卖家专属决策记忆层<br/>profile + SOP + decisions + confirmed learned<br/>跨工具 · 带理由 · 可确认 · 可回滚"]
...
> 卖家已经不缺工具：卖家精灵和 SIF 告诉你市场长什么样，领星保存自己的经营数据，LinkFox 和翻译解决内容，虎步可以批量搬报表。但一次选品真正拍板时，还需要回答：这个机会适不适合我，上次为什么拒绝类似产品，那次踩坑今天要不要继续生效？这些理由分散在表格、会议和微信里，很少以可确认、可回滚的方式进入下一轮排序。我们补的是这一层。接下来演示同一批候选如何经过卖家画像过滤，以及三条重复拒绝如何先生成待确认规则；只有人工确认后，下一轮排序才改变。
...
2. 三条不同决策证据才生成 learned 候选；
3. `confirmed: false` 不改变排序；
4. 人工确认后重跑，排序发生可见变化；
5. 每个候选都有双向个性化归因；
